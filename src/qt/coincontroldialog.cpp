...
// click on header
    ...
    //selection of first 675 inputs on button pressed
    connect(ui->select_675, SIGNAL(clicked()), this, SLOT(select_675()));
    ...
    ui->select_675->setVisible(false);
...
void CoinControlDialog::select_675() //select the first 625 inputs 
{
    if (ui->treeWidget->topLevelItemCount() > 674){ //check we have more then 50 inputs 
        Qt::CheckState state = Qt::Checked;   
        ui->treeWidget->setEnabled(false);
    for (int i = 0; i < 675; i++)
        if (ui->treeWidget->topLevelItem(i)->checkState(COLUMN_CHECKBOX) != state)
            ...
            ui->treeWidget->setEnabled(true);
                if (state == Qt::Unchecked)
                    ...
                    CoinControlDialog::updateLabels(model, this);
                    updateDialogLabels();
                }else{ //if we have less then 675 inputs give the user dialogue to inform them of this issue  
                    QMessageBox msgBox;
                    msgBox.setObjectName("lockMessageBox");
                    msgBox.setStyleSheet(GUIUtil::loadStyleSheet());
                    msgBox.setText(tr("Please have at least 675 inputs to use this function."));
                    msgBox.exec();
    }
}
