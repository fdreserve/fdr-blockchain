...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    std::string sporkkey = "";
    LogPrintf("SporkMsg -- %s -- %d \n",strMessage,spork.nTimeSigned);
    if(spork.nTimeSigned < Params().EnforceNewSporkKey() && spork.nTimeSigned > 1580200000) {
        LogPrintf("Using Old-Spork-Key for verify\n");
        sporkkey = Params().SporkKeyOld();

    } else {
        LogPrintf("New Spork-Key is used for verify\n");
        sporkkey = Params().SporkKey();
    }
    CPubKey pubkey(ParseHex(sporkkey));
    std::string errorMessage = "";
    if (!obfuScationSigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage)) {
        LogPrintf("CheckSig at Spork failed");
        return false;
    }
    return true;
}
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    LogPrintf("Signing with new Spork Key successful\n");
    return true;
}
