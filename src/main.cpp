    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("CheckBlock() : Couldn't find masternode payment"), REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            if (fDebug)
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (block.GetHash() != Params().HashGenesisBlock() && !CheckWork(block, pindexPrev)) {
        LogPrintf("Main::3558 Check Work of block hash\n");
        return false;
    }
    ...
    if (!AcceptBlockHeader(block, state, &pindex)) {
        LogPrintf("Main::3577 AccpetBlockHeader was false\n");
        return false;
    }
    ...
    if (pindex->nStatus & BLOCK_HAVE_DATA) {
        ...
        LogPrintf("Main::3580 Already have block\n");
        return true;
    }
    ...
    if( nHeight > Params().GetV221ActivationHeight()) {
        if (isPoS) {
            ...
            // Check for serial double spent on the same block
            for (const CTransaction& tx : block.vtx) {
                for (const CTxIn& in: tx.vin) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (pblock->GetHash() != Params().HashGenesisBlock() && pfrom != NULL) {
        ...
        if (mi == mapBlockIndex.end()) {
            ...
            LogPrintf("Main::3796 Prev Block is not our prev block\n");
            return false;
        }
    }

    {
        ...
        // stake input check for prevent "Spent Stake" vulnerability check
